...
const PokemonDetailsPage: React.FC<PokemonDetailsPageProps> = () => {
  const router = useRouter();
  const userName = router.query.pokemon?.toString() || "";

  const { user, userLoading, mutateUser } = useUser(userName);

  const [nickname, setNickname] = useState("");

  useEffect(() => {
    setNickname(""); // Reset nickname when the pokemon changes
  }, [user]);
  ...
  const handleSubmitNickname = async (e: React.FormEvent) => {
    ...
    const update = await UserApi.setNickname(user, nickname.trim());
  ...
  return (
    <>
      <Head>
        {user && <title>{`${user?.firstName} - NextJS PokéDex`}</title>}
      </Head>
            ...
            <div className="col-span-4 sm:col-span-3">
              <div className="bg-white shadow rounded-lg p-6">
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-gray-700">{user?.company?.department} </p>
